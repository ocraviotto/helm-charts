...
| museum.image | object | `{"pullPolicy":"IfNotPresent","registry":"ghcr.io","repository":"ocraviotto/container-images/ente-server","tag":"0.0.20250815-69.1"}` | The upstream image is built once a week. This image is built every time there is a new server deployed at ente.io as detected by [renovate](https://github.com/ocraviotto/container-images/blob/main/renovate.json) and the [Github workflow here](https://github.com/ocraviotto/container-images/blob/main/.github/workflows/build-images.yml). curl -s https://api.ente.io/ping | jq '.id' |
...
| museum.image.repository | string | `"ocraviotto/container-images/ente-server"` | My own repo to ensure timely builds [ghcr.io repo](https://github.com/ocraviotto/container-images/pkgs/container/container-images%2Fente-server). |
| museum.image.tag | string | `"0.0.20250815-69.1"` | If empty, it uses the chart appVersion, based on the latest docker releases in the image [Github Packages](https://github.com/ocraviotto/container-images/pkgs/container/container-images%2Fente-server) These tags are generated based on time build and build number + attempt. Read the image metadata to know what hash revision is built from: e.g. `skopeo inspect docker://ghcr.io/ocraviotto/container-images/ente-server:0.0.20250814-1 | jq '.Labels."org.opencontainers.image.revision"' For details see: https://github.com/ocraviotto/container-images/blob/main/specs/ente.yml |
...
| web.image.repository | string | `"ocraviotto/container-images/ente-web"` | My own repo to ensure timely builds [ghcr.io repo](https://github.com/ocraviotto/container-images/pkgs/container/container-images%2Fente-web). |
| web.image.tag | string | `"0.0.20250815-69.1"` | Can be a tag or digest. The image is built every time we trigger a server build, and the source uses ente's main branch. The tags are generated based on time build and build number + attempt. Read the image metadata to know what hash revision is built from: e.g. `skopeo inspect docker://ghcr.io/ocraviotto/container-images/ente-web:0.0.20250814-1 | jq '.Labels."org.opencontainers.image.revision"'` For details see: https://github.com/ocraviotto/container-images/blob/main/specs/ente.yml |
