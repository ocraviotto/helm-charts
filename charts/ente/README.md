...
![Version: 0.2.2](https://img.shields.io/badge/Version-0.2.2-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 0.0.20260811-318.1](https://img.shields.io/badge/AppVersion-0.0.20260811--318.1-informational?style=flat-square)
...
```bash
helm install home-ente oci://ghcr.io/ocraviotto/charts/ente --version 0.2.2
```
...
| museum.image.tag | string | `"0.0.20260811-318.1"` | If empty, it uses the chart appVersion, based on the latest Docker image release in the image [GitHub Packages](https://github.com/ocraviotto/container-images/pkgs/container/container-images%2Fente-server)</br> These tags are generated based on time build and build number + attempt. Read the image metadata to know what hash revision is built from: e.g. `skopeo inspect docker://ghcr.io/ocraviotto/container-images/ente-server:0.0.20250814-1 \| jq '.Labels."org.opencontainers.image.revision"'`</br> For details see: <https://github.com/ocraviotto/container-images/blob/main/specs/ente.yml> |
...
| web.image | object | `{"pullPolicy":"IfNotPresent","registry":"ghcr.io","repository":"ocraviotto/container-images/ente-web","tag":"0.0.20260811-318.1"}` | The upstream image is built once a week. This image is built every time there is a new server deployed at ente.io as detected by [renovate](https://github.com/ocraviotto/container-images/blob/main/renovate.json) and the [GitHub workflow here](https://github.com/ocraviotto/container-images/blob/main/.github/workflows/build-images.yml). |
